...
#include zephyrlogginglog.h
LOG_MODULE_REGISTER(modem_gsm, CONFIG_MODEM_LOG_LEVEL);

#include stdlib.h
#include zephyrkernel.h
#include zephyrdevice.h
#include zephyrsysring_buffer.h
#include zephyrsysutil.h
#include zephyrnetppp.h
#include zephyrdriversmodemgsm_ppp.h
#include zephyrdriversuart.h
#include zephyrdriversconsoleuart_mux.h

#include modem_context.h
#include modem_iface_uart.h
#include modem_cmd_handler.h
#include ..consolegsm_mux.h
...
static int gsm_connect_flag = 0 ;

	 Handler +QLTS time[0-21],dst[22]

MODEM_CMD_DEFINE(on_cmd_local_time_qlts)
{
	size_t out_len;

	out_len = net_buf_linearize(gsm.minfo.mdm_localtime, sizeof(gsm.minfo.mdm_localtime) - 1,
				    data-rx_buf, 0, len);
	gsm.minfo.mdm_localtime[out_len] = '0';
	LOG_INF(LOCAL TIME %s, gsm.minfo.mdm_localtime);
	k_msleep(100);
	gsm_connect_flag = 1;
...
int year_1byte()
{
	printk(character  = %cn, gsm.minfo.mdm_localtime[8]);
	int yr_1=(int)((char)(gsm.minfo.mdm_localtime[8]-48));
	int yr_2=(int)((char)(gsm.minfo.mdm_localtime[9]-48));
	int yr_3=(int)((char)(gsm.minfo.mdm_localtime[10]-48));
	int yr_4=(int)((char)(gsm.minfo.mdm_localtime[11]-48));
	return (int)(yr_11000 + yr_2100 + yr_310 + yr_4 - 1900);
}

int mon_1byte()
{
	int mon1=(int)((char)(gsm.minfo.mdm_localtime[13]-48));
	int mon2=(int)((char)(gsm.minfo.mdm_localtime[14]-48));
	return (int)(mon110 + mon2 - 1 );
}

int date_1byte()
{
	int date1=(int)((char)(gsm.minfo.mdm_localtime[16]-48));
	int date2=(int)((char)(gsm.minfo.mdm_localtime[17]-48));
	return (int)(date110 + date2  );
}

int hour_1byte()
{
	int hr1=(int)((char)(gsm.minfo.mdm_localtime[19]-48));
	int hr2=(int)((char)(gsm.minfo.mdm_localtime[20]-48));
	return (int)(hr110 + hr2  );
}

int min_1byte()
{
	int a=(int)((char)(gsm.minfo.mdm_localtime[22]-48));
	int b=(int)((char)(gsm.minfo.mdm_localtime[23]-48));
	return (int)(a10 + b );
}

int sec_1byte()
{
	int c=(int)((char)(gsm.minfo.mdm_localtime[25]-48));
	int d=(int)((char)(gsm.minfo.mdm_localtime[26]-48));
	return (int)(c10 + d  );
}

static const struct setup_cmd cmd_local_time_cmds[] =
{
	 query local time (sync) to network 	
	SETUP_CMD(AT+QLTS=1,, on_cmd_local_time_qlts, 0U, )
};

static int gsm_query_local_time(struct gsm_modem gsm)
{
	...
	ret =  modem_cmd_handler_setup_cmds_nolock(&gsm-context.iface,
						  &gsm-context.cmd_handler,
						  cmd_local_time_cmds,
						  ARRAY_SIZE(cmd_local_time_cmds),
						  &gsm-sem_response,
						  GSM_CMD_SETUP_TIMEOUT);
...
MODEM_CMD_DEFINE(on_cmd_atcmdinfo_rssi_cesq)
{
	...
	if (rsrp = 0 && rsrp = 97) {
		...
		LOG_INF(RSRPRSCPRSSI not known);
	}
...
static const struct setup_cmd setup_modem_info_cmds[] = {
	...
	SETUP_CMD(AT+CGSN, , on_cmd_atcmdinfo_imei, 0U, ),
	 ...
	 sanket ---
	SETUP_CMD(AT+QLTS,, on_cmd_local_time_qlts, 0U, ),
...
{
	 Expected response is +CGATT 01 so simply look for '1' 
...
static int gsm_query_modem_info(struct gsm_modem gsm)
{
	...
	ret =  modem_cmd_handler_setup_cmds_nolock(&gsm-context.iface,
						  ...
						  &gsm-sem_response,
						  GSM_CMD_SETUP_TIMEOUT);
	int system_start_time = gsm.minfo.mdm_localtime + k_uptime_get(); 
